    ...
    if(event->type() == QEvent::MouseButtonPress && nobird == 0)
       {
           if(birdsitua == 0){   //在彈弓上
               click = 1; //點
               mousepress_x = (mouseEvent->pos().x() - 23) / 800.0 * 32.0;  //抓點擊的座標並換算成BOX2D的
       ...
       if(event->type()==QEvent::MouseMove && click>=1 && nobird == 0)   //已經點著
       {
           if(birdsitua==1 || birdsitua==2){   //點或拉
               float coor_x = (mouseEvent->pos().x() - 23) / 800.0 * 32.0;
               float coor_y = 18.0-( (mouseEvent->pos().y()-46) * 18 / 600.0);

              //if( (inisite_x - coor_x)*(inisite_x - coor_x) + (inisite_y - coor_y)*(inisite_y - coor_y) <= 4){
       ...
       if(event->type() == QEvent::MouseButtonRelease && nobird == 0)
       {
         if(birdsitua == 2){    //拉
             ...
             if(birdList[color]->getcolor() == 0){   //紅鳥
...
/*void MainWindow::deletepig(){
    sitep1 = pig_one->getBody().GetPosition().x;
    sitep2 = pig_two->getBody().GetPosition().x;
    if( sitep1 < 400 || sitep1 > 550 ){   //473
         pigonelife = 0;
    }
    if(pigonelife == 0){
        delete pig_one;
    }
    if( sitep2 < 550 || sitep2 > 700 ){   //623
         pigtwolife = 0;
    }
    if(pigtwolife == 0){
        delete pig_two;
...
void MainWindow::initial(){
    ...
    Bird *birdie = new Bird(8.0f,7.0f,0.70f,&timer,QPixmap(":/bird.png").scaled(height()/9.0,height()/9.0),world,scene);   //position in BOX2D x, y, radius
    // Setting the Velocity
    itemList.push_back(birdie);
    birdList.push_back(birdie);

    yellow_bird *birdie_yellow = new yellow_bird(4.0f,2.50f,0.70f,&timer,QPixmap(":/image/1321329378-3367254444.png").scaled(height()/9.0,height()/9.0),world,scene);
    itemList.push_back(birdie_yellow);
    birdList.push_back(birdie_yellow);

    Black_bird *birdie_black = new Black_bird(2.0f,2.50f,0.85f,&timer,QPixmap(":/image/pajarito5.png").scaled(height()/9.0,height()/9.0),world,scene);
    itemList.push_back(birdie_black);
    birdList.push_back(birdie_black);

    Blue_bird *birdie_blue = new Blue_bird(0.0f,2.50f,0.90f,&timer,QPixmap(":/image/pajarito2.png").scaled(height()/9.0,height()/9.0),world,scene);
    itemList.push_back(birdie_blue);
    birdList.push_back(birdie_blue);

    //create pig
    pig_one = new pig(18.0f,6.0f,0.87f,&timer,QPixmap(":/image/20150810032747230.png").scaled(height()/9.0,height()/9.0),world,scene);
    itemList.push_back(pig_one);
    pig_two = new pig(24.0f,8.0f,0.90f,&timer,QPixmap(":/image/Pig_sneeze_copy.png").scaled(height()/9.0,height()/9.0),world,scene);
    ...
    //create slingshot
    itemList.push_back(new slingshot(8,6,0.001,0.001,QPixmap(":/image/sling_shot_vector_by_birdalliance-d84p8sd.png").scaled(width()/17,height()/4.0),world,scene));
    scene->update();
}
